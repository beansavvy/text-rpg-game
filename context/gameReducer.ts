...
export type GameAction =
  | {
      ...
      character: GameCharacterState;
      addToParty?: boolean;
    }
  | {
      type: 'UPDATE_CHARACTER';
      characterId: string;
      changes: Partial<GameCharacterState>;
    }
  | {
      type: 'SET_CHARACTER_RESOURCE';
      characterId: string;
      resource: 'health' | 'stamina' | 'mana' | 'experience';
      current: number;
    }
...
export const gameReducer = (
  ...
  switch (action.type) {
    case 'CREATE_CHARACTER': {
      const memberIds =
        action.addToParty &&
        !state.party.memberIds.includes(action.character.id)
    ...
    case 'UPDATE_CHARACTER': {
      ...
      return {
        ...
        characters: {
          ...state.characters,
          [action.characterId]: { ...character, ...action.changes },
        },
      };
    }
      ...
      return {
        ...
        characters: {
          ...
          [action.characterId]: {
            ...
            [action.resource]: {
              ...resource,
              current: Math.max(0, Math.min(action.current, resource.max)),
            },
          },
        },
      };
    ...
    case 'ADD_PARTY_MEMBER':
      if (
        ...
        state.party.memberIds.includes(action.characterId)
      )
        return state;
      return {
        ...state,
        party: {
          ...state.party,
          memberIds: [...state.party.memberIds, action.characterId],
        },
      };
    case 'REMOVE_PARTY_MEMBER':
      return {
        ...
        party: {
          ...state.party,
          memberIds: state.party.memberIds.filter(
    ...
    case 'UPDATE_PARTY_GOLD':
      return {
        ...
        party: {
          ...state.party,
          gold: Math.max(0, state.party.gold + action.amount),
        },
      };
    case 'ADD_INVENTORY_ITEM': {
      const quantity = action.quantity ?? 1;
      const existing = state.party.inventory.find(
        (item) => item.itemId === action.itemId && !item.equipped
      );
      const inventory = existing
        ? state.party.inventory.map((item) =>
            item === existing
          ...
          inventory: state.party.inventory.map((item) =>
            item.itemId === action.itemId
              ? {
                  ...
                  characterEquippedId: action.equipped
    ...
    case 'DISCOVER_LOCATION':
      if (state.world.discoveredLocationIds.includes(action.locationId))
        return state;
      return {
        ...state,
        world: {
    ...
    case 'SET_ACTIVE_NPC':
      return {
        ...state,
        interaction: { ...state.interaction, activeNpcId: action.npcId },
      };
      ...
      return {
        ...
        combat: {
          ...
          enemy: state.combat.enemy
            ? { ...state.combat.enemy, ...action.changes }
            : state.combat.enemy,
        },
      };
